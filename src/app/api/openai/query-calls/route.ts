...
const detectQueryType = (
  ...
  const isKeywordSearch = keywordSearchPatterns.some((pattern) =>
    pattern.test(query)
  );
  ...
  if (isKeywordSearch) {
    ...
    lowerQuery.includes("disposition") ||
...
const extractKeywordsFromQuery = (query: string): string[] => {
  ...
  if (quotedPhrases) {
    quotedPhrases.forEach((phrase) => {
      ...
      if (cleaned.length > 0) {
        ...
        const phraseWords = cleaned
          .split(/\s+/)
          .filter((word) => word.length > 2);
        phraseWords.forEach((word) => {
          const lowerWord = word.toLowerCase();
          if (
            !processedTerms.has(lowerWord) &&
            (word.length > 3 ||
              [
                "fee",
                "pay",
  ...
  cleanQuery = cleanQuery
    .toLowerCase()
    .replace(
      /\b(how many|count|find|search|show me|what|where|when|calls?|records?|included?|contain|mentioned?|about|with|have|were|that|this|they|them|from|call|and|or|the|a|an|is|are|was|be|been|being|for|offer|offers|offered)\b/gi,
      ""
    )
    .replace(/[^\w\s]/g, " ")
    .trim();
  ...
  for (let i = 0; i < words.length - 1; i++) {
    ...
    if (phrase2.length > 5 && !processedTerms.has(phrase2.toLowerCase())) {
      const isAutomotivePhrase =
        /\b(wheel|tire|brake|oil|engine|alignment|service|repair|quote|warranty|appointment|battery|transmission|filter|fluid)\b/i.test(
          phrase2
        );
      const isServicePhrase =
        /\b(customer|billing|account|refund|cancel|support|help|delivery|payment|schedule)\b/i.test(
          phrase2
        );
      ...
      if (
        isAutomotivePhrase ||
        isServicePhrase ||
        phrase2.split(" ").every((w) => w.length > 4)
      ) {
    ...
    if (
      ...
      phrase3.length > 10 &&
      phrase3.length < 30 &&
      !processedTerms.has(phrase3.toLowerCase())
    ) {
      const isAutomotivePhrase =
        /\b(wheel|tire|brake|oil|engine|alignment|service|repair|quote|warranty|appointment|battery|transmission|filter|fluid)\b/i.test(
          phrase3
  ...
  return uniqueKeywords
    .sort((a, b) => {
      ...
      if (
        quotedPhrases?.some((p) => p.includes(a)) &&
        !quotedPhrases?.some((p) => p.includes(b))
      )
        return -1;
      if (
        quotedPhrases?.some((p) => p.includes(b)) &&
...
const performKeywordSearch = (
  ...
  console.log(
    `🔍 Performing ENHANCED keyword search for: ${searchTerms.join(", ")}`
  );
  ...
  console.log(
    `📈 Expanded ${searchTerms.length} search terms to ${allVariations.size} total variations:`
  );
  console.log(`   Search terms: ${searchTerms.join(", ")}`);
  console.log(
    `   All variations: ${Array.from(allVariations).slice(0, 20).join(", ")}${
      allVariations.size > 20 ? "..." : ""
    }`
  );

  const recordsWithTranscripts = records.filter(
    (record) => record.transcript_text && record.transcript_text.trim().length
  );

  console.log(
    `📊 Total records: ${records.length}, Records with transcripts: ${recordsWithTranscripts.length}`
  );
    ...
    searchTerms.forEach((originalTerm) => {
      ...
      variations.forEach((variation) => {
        ...
        if (matches) {
          ...
          while (
            (match = snippetRegex.exec(transcript)) !== null &&
            matchingSnippets.length < 4
          ) {
            ...
            if (
              snippet.length > 15 &&
              !matchingSnippets.some((existing) =>
                existing.includes(snippet.substring(10, 30))
              )
            ) {
  ...
  const result: KeywordSearchResult = {
    ...
    searchStats: {
      ...
      matchPercentage:
        recordsWithTranscripts.length > 0
          ? (matchingRecords.length / recordsWithTranscripts.length) * 100
          : 0,
    },
  };
  ...
  console.log(
    `   - Match percentage: ${result.searchStats.matchPercentage.toFixed(1)}%`
  );
...
const selectRelevantTranscripts = (
  ...
  const recordsWithLongTranscripts = records.filter(
    (record) =>
      record.transcript_text && record.transcript_text.trim().length > 50
  );

  console.log(`Records with transcript_text: ${recordsWithTranscripts.length}`);
  console.log(
    `Records with transcript_text >50 chars: ${recordsWithLongTranscripts.length}`
  );

  const availableTranscripts = records.filter(
    (record) => record.transcript_text
  );

  console.log(
    `Available transcripts after filtering: ${availableTranscripts.length}`
  );
  ...
  const scoredTranscripts = availableTranscripts.map((record) => {
    ...
    queryKeywords.forEach((keyword) => {
      const matches = (transcriptLower.match(new RegExp(keyword, "g")) || [])
        .length;
  ...
  console.log(
    `✅ Selected ${selected.length} transcripts for query type: ${queryType}`
  );
...
const prepareContextForQuery = (
  ...
  console.log(
    `🔍 Preparing context for detected query type: ${detectedQueryType}`
  );
  ...
  if (detectedQueryType === "keyword_search" && keywordSearchResults) {
    const totalCallsPercentage =
      (keywordSearchResults.matchingRecords.length / metrics.totalCalls) * 100;
    const transcriptCallsPercentage =
      keywordSearchResults.searchStats.matchPercentage;
    ...
    context += `**Expanded to ${
      keywordSearchResults.expandedTerms.length
    } Variations:** ${keywordSearchResults.expandedTerms
      .slice(0, 15)
      .join(", ")}${
      keywordSearchResults.expandedTerms.length > 15 ? "..." : ""
    }\n`;
    context += `**CALL COUNT WITH KEYWORDS:** ${keywordSearchResults.matchingRecords.length} calls\n`;
    context += `**PERCENTAGE OF TOTAL CALLS:** ${totalCallsPercentage.toFixed(
      1
    )}% (${keywordSearchResults.matchingRecords.length} out of ${
      metrics.totalCalls
    } total calls)\n`;
    context += `**PERCENTAGE OF CALLS WITH TRANSCRIPTS:** ${transcriptCallsPercentage.toFixed(
      1
    )}% (${keywordSearchResults.matchingRecords.length} out of ${
      keywordSearchResults.searchStats.recordsWithTranscripts
    } calls with transcript data)\n`;
    ...
    if (keywordSearchResults.matchingRecords.length > 0) {
      ...
      keywordSearchResults.matchingRecords
        .slice(0, 10)
        .forEach((match, index) => {
          ...
          context += `**Duration:** ${Math.round(
            match.duration / 60
          )}m | **Keyword Occurrences:** ${match.matchCount}\n`;
          context += `**Matched Variations:** ${match.matchedVariations
            .slice(0, 5)
            .join(", ")}${match.matchedVariations.length > 5 ? "..." : ""}\n`;
    ...
    const transcriptSamples = selectRelevantTranscripts(
      rawRecords,
      detectedQueryType,
      query,
      3
    ...
    if (transcriptSamples.length > 0) {
      ...
      transcriptSamples.forEach((sample, index) => {
        ...
        context += `**Agent:** ${sample.agent} | **Disposition:** ${
          sample.disposition
        } | **Sentiment:** ${sample.sentiment} | **Duration:** ${Math.round(
          sample.duration / 60
        )}m\n`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `🏷️ Extracted keywords: ${detectedQuery.extractedKeywords.join(", ")}`
    );

    // Debug the available data
    console.log(`📊 Data available:`);
    console.log(
      `  - fullRecords: ${fullRecords ? fullRecords.length : "null"} records`
    );
    console.log(
      `  - callData?.data?.sampleRecords: ${
        callData?.data?.sampleRecords
          ? callData.data.sampleRecords.length
          : "null"
      } records`
    );
    console.log(
      `  - callData?.data?.fullRecords: ${
        callData?.data?.fullRecords ? callData.data.fullRecords.length : "null"
      } records`
    );
    ...
    if (actualQueryType === "keyword_search" && detectedQuery.isKeywordSearch) {
      ...
      if (fullRecords && fullRecords.length > 100) {
        ...
        console.log(
          `✅ Using fullRecords for keyword search: ${allAvailableRecords.length} records`
        );
      } else if (
        callData?.data?.fullRecords &&
        callData.data.fullRecords.length > 100
      ) {
        allAvailableRecords = callData.data.fullRecords;
        console.log(
          `✅ Using callData.fullRecords for keyword search: ${allAvailableRecords.length} records`
        );
      } else if (callData?.data?.sampleRecords) {
        allAvailableRecords = callData.data.sampleRecords;
        console.log(
          `⚠️ WARNING: Only sample records available for keyword search: ${allAvailableRecords.length} records`
        );
        console.log(
          `⚠️ This will give inaccurate keyword counts. Full dataset needed for accurate results.`
        );
      } else {
        return NextResponse.json(
          {
            error:
              "Insufficient data for keyword search. Full dataset required for accurate keyword counting.",
            suggestion:
              "Please ensure all call records are sent to the backend for keyword searches.",
          },
          { status: 400 }
        );
      }
    ...
    if (
      actualQueryType === "keyword_search" &&
      detectedQuery.extractedKeywords.length > 0
    ) {
      console.log(
        `🔍 Performing ENHANCED keyword search on ALL ${allAvailableRecords.length} records...`
      );

      // Search ALL available records for accurate counts with enhanced matching
      keywordSearchResults = performKeywordSearch(
        allAvailableRecords,
        detectedQuery.extractedKeywords
      );
      console.log(`✅ Enhanced keyword search results:`);
      console.log(
        `   - Original terms: ${keywordSearchResults.searchTerms.length}`
      );
      console.log(
        `   - Expanded variations: ${keywordSearchResults.expandedTerms.length}`
      );
      console.log(`   - Total matches: ${keywordSearchResults.totalMatches}`);
      console.log(
        `   - Records with matches: ${keywordSearchResults.matchingRecords.length}`
      );
      console.log(
        `   - Records searched: ${keywordSearchResults.searchStats.totalRecordsSearched}`
      );
      console.log(
        `   - Records with transcripts: ${keywordSearchResults.searchStats.recordsWithTranscripts}`
      );

      // For AI processing, create a smart subset
      if (keywordSearchResults.matchingRecords.length > 0) {
        const matchingRecordIds = new Set(
          keywordSearchResults.matchingRecords.map((r) => r.id)
        );
        const matchingRecords = allAvailableRecords.filter((r) =>
          matchingRecordIds.has(r.id)
        );
        const nonMatchingRecords = allAvailableRecords.filter(
          (r) => !matchingRecordIds.has(r.id)
        );
        ...
        recordsToAnalyze = [
          ...limitedMatchingRecords,
          ...sampleNonMatchingRecords,
        ];

        console.log(
          `📋 Records for AI analysis: ${recordsToAnalyze.length} (${limitedMatchingRecords.length} matching + ${sampleNonMatchingRecords.length} sample)`
        );
      } else {
        // No matches found, use a sample for baseline metrics
        recordsToAnalyze = allAvailableRecords;
        console.log(
          `📋 No matches found. Using ${recordsToAnalyze.length} sample records for baseline metrics.`
        );
      }
    } else {
      ...
      if (allAvailableRecords.length > maxRecordsForAnalysis) {
        ...
        console.log(
          `📊 Limited to ${maxRecordsForAnalysis} records for non-keyword analysis`
        );
      }
    }

    console.log(
      `Processing ${recordsToAnalyze.length} call records for AI analysis (detected query type: ${actualQueryType})`
    );
    ...
    if (actualQueryType === "keyword_search" && keywordSearchResults) {
      ...
      console.log(
        `📈 Metrics updated: totalCalls set to ${metrics.totalCalls} (complete dataset)`
      );
    }
...
Please provide a comprehensive analysis with specific metrics and actionable insights. When enhanced keyword search results are available, explain the scope of matching used and use the examples to validate and illustrate your findings.`;
    ...
    const transcriptCount = recordsToAnalyze.filter((record) => {
      return (
        record.transcript_text && typeof record.transcript_text === "string"
      );
    }).length;
